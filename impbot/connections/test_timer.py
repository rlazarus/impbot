...
class MockThreadingTimer(threading.Timer):
    ...
    def run(self):
        # RepeatingTimer schedules new lambdas on time, without waiting for the
        # run function to complete. When we mock out the actual delay for unit
        # testing, that's basically running in a tight loop, so lambdas stack up
        # faster than they get executed. (Where that actually turns into a
        # problem is, the end_time overflows the maximum date, which is very
        # funny.) We join the queue here to block until the last timer execution
        # has completed before we start the next one.
        self.queue.join()
...
class TestTimer(unittest.TestCase):
    def setUp(self):
        ...
        self.queue: queue.Queue[
            Union[lambda_event.LambdaEvent, bot.Shutdown]] = queue.Queue()
        ...
        self.timer_thread = threading.Thread(target=self.timer_conn.run,
                                        args=(self.queue.put,))
