...
class ValePointsHandler(base.Handler[twitch_eventsub.PointsRewardRedemption]):
    ...
    def run(self, event: twitch_eventsub.PointsRewardRedemption) -> Optional[str]:
        if event.reward_title.startswith('Emote only mode'):
            ...
            return f'@Vale {event.user} redeemed an Extra Hello with No-Bamboozle Insurance!'
        else:
            return None
    ...
    def emote_only(self, event: twitch_eventsub.PointsRewardRedemption) -> str:
        if not self.timer or not self.timer.active():
            ...
            return f'{event.user} redeemed emote-only mode for two minutes! valePanic'
        else:
    ...
    def vip(self, event: twitch_eventsub.PointsRewardRedemption) -> str:
        ...
        stream_data = self.twitch_util.get_stream_data(username=self.twitch_util.streamer_username)
    ...
    def movie_night(self, event: twitch_eventsub.PointsRewardRedemption) -> str:
        ...
        if response.status_code != 200:
            ...
            return (f'@{event.user} tried to redeem a movie night pass, but something went wrong. '
                    'valeS')
        json = response.json()
        if not json['success']:
            logger.error(json)
            return (f'@{event.user} tried to redeem a movie night pass, but something went wrong. '
                    'valeRIP')
        logger.info(json)
        return f'@{event.user} redeemed a movie night pass! See you there! valeCool'


class ValePointsCleanupObserver(base.Observer[twitch_eventsub.StreamStartedEvent]):
...
def module_group(twitch_conn: twitch.TwitchChatConnection, timer_conn: timer.TimerConnection,
