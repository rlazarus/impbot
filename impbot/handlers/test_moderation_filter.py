...
class ModerationFilterHandlerTest(tests_util.DataHandlerTest):
    ...
    def tearDown(self):
        d = data.Namespace(
            "impbot.handlers.moderation_filter.ModerationFilterHandler")
        d.unset("permitted")
        d.unset("warning")
        super().tearDown()

    def message(
            self, text: str, user: twitch.TwitchUser = None,
            action: bool = False,
        ...
        if not emotes:
            emotes = []
        return twitch.TwitchMessage(self.conn, user, text, "id", None, 12345,
                                    action, emotes)

    def assert_allowed(self, text: str, user: twitch.TwitchUser = None,
                       action: bool = False,
    ...
    def assert_blocked(self, text: str, user: twitch.TwitchUser = None,
                       action: bool = False,
    ...
    def assert_delete(self, reply_substring: str, text: str,
                      user: twitch.TwitchUser = None, action: bool = False,
                      emotes: List[Tuple[str, int, int]] = None):
        self.assert_blocked(text, user, action, emotes)
    ...
    def test_link(self):
        ...
        self.assert_timeout("post a link", 180,
                            "a link.org in the middle of a line")
    ...
    def test_permit_alwaysallowed(self):
        ...
        mod = twitch.TwitchUser("mod", display_name="Mod", is_moderator=True)
        self.assertTrue(
            self.permit_handler.check(self.message("!permit alloweduser", mod)))
        self.assertEqual(
            self.permit_handler.run(self.message("!permit alloweduser", mod)),
