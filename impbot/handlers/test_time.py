...
class TimeHandlerTest(tests_util.DataHandlerTest):
    ...
    def test_no_event(self):
        ...
        self.assert_response('!time', "@username You've spent 6 minutes in the chat.")
        self.assert_response('!time username', "@username You've spent 6 minutes in the chat.")
        self.assert_response('!time username', 'Username has spent 6 minutes in the chat.',
                             user=base.User('another_user'))
        self.assert_response('!time olduser', 'olduser spent 7 minutes in the chat.')
    ...
    def test_event(self):
        ...
        self.assert_response(
            '!time',
            "@username You've spent 6 minutes in the chat (5 minutes during the Arbor Day event).")
        self.assert_response(
            '!time username',
            "@username You've spent 6 minutes in the chat (5 minutes during the Arbor Day event).")
        self.assert_response(
            '!time username',
            'Username has spent 6 minutes in the chat (5 minutes during the Arbor Day event).',
            user=base.User('another_user'))
        self.assert_response('!time olduser', 'olduser spent 7 minutes in the chat.')
