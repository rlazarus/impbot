...
class CommandTest(unittest.TestCase):
    ...
    def testOneOptionalStringArg(self):
        ...
        self.assertEqual(['foo bar'], command._args([Optional[str]], self.x, 'foo bar'))
    ...
    def testSeveralRequired(self):
        ...
        self.assertEqual([6, 'foo', 42.0], command._args([int, str, float], self.x, '6 foo 42'))
        self.assertRaises(command.UsageError, command._args, [int, str, float], self.x, '6 42')
        self.assertRaises(command.UsageError, command._args, [int, str, float], self.x, '6  42')
        self.assertRaises(
            command.UsageError, command._args, [int, str, float], self.x, '6 foo bar 42')

        # A string at the end must be at least one word.
        self.assertEqual([6, 42.0, 'foo'], command._args([int, float, str], self.x, '6 42 foo'))
        self.assertEqual(
            [6, 42.0, 'foo bar'], command._args([int, float, str], self.x, '6 42 foo bar'))
        self.assertRaises(command.UsageError, command._args, [int, float, str], self.x, '6 42')
        self.assertRaises(command.UsageError, command._args, [int, float, str], self.x, '6 42 ')

    def testTrailingOptionals(self):
        self.assertEqual(
            [6, None, None], command._args([int, Optional[int], Optional[str]], self.x, '6'))
        self.assertEqual(
            [6, 12, None], command._args([int, Optional[int], Optional[str]], self.x, '6 12'))
        self.assertEqual(
            [6, 12, 'foo'], command._args([int, Optional[int], Optional[str]], self.x, '6 12 foo'))
        self.assertEqual([6, 12, 'foo bar'],
                         command._args([int, Optional[int], Optional[str]], self.x, '6 12 foo bar'))
        self.assertRaises(
            command.UsageError, command._args, [int, Optional[int], Optional[str]], self.x, '6 foo')
    ...
    def testUnionFallback(self):
        ...
        self.assertRaises(command.UsageError, command._args, [Union[int, str]], self.x, '')
        self.assertRaises(command.UsageError, command._args, [Union[int, float]], self.x, 'two')

    def testExceptionText(self):
        self.assertRaisesRegex(
            command.UsageError, r'^Usage: !x <arga> <argb>$', command._args, [str], self.x, '')
        self.assertRaisesRegex(command.UsageError, r'^Usage: !y <arga> \[<argb>\] \[<argc>\]$',
                               command._args, [str], self.y, '')
        self.assertRaisesRegex(command.UsageError, r'^Usage: !z <explicit usage string>$',
